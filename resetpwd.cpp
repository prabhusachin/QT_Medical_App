    ...
    ui(new Ui::ResetPwd)
{
    ...
    this->setModal(true);
    sem = seml;
    ui->btnUpd->setStyleSheet("background-color: rgb(255,237,208)");
}
...
void ResetPwd::on_btnUpd_clicked()
{
    ...
    if(!pwd.isEmpty() && !cpwd.isEmpty())
    {
        if(pwd == cpwd)
        {
            ...
            QSqlQuery query(db);
            query.prepare(QString("update CustomerReg set stpass=:stpass,stcpass=:stcpass where stemail=:stemail"));
