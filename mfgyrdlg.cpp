#include "mfgyrdlg.h"
#include "ui_mfgyrdlg.h"

MfgYrDlg::MfgYrDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::MfgYrDlg)
{
    ui->setupUi(this);
...
void MfgYrDlg::initializeModel(QStandardItemModel *model1)
{
...
void MfgYrDlg::on_pushButton_clicked()
{
    ...
    r=0;
         for(int m=1;m<=12;m++)
         {
             ...
             if(query.size()!=0)
             {
              //  QMessageBox::warning(0,"dsd",QString::number(catid));
             while (query.next())
             {
                 r++;
             }
    ...
    void MfgYrDlg::createView(QStandardItemModel *model4)
    {
        ui->tableView->setModel(model4);
    }

void MfgYrDlg::on_btnDelete_clicked()
{
